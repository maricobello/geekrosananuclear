...
st.markdown("<h1 style='text-align: center; color: white;'>Sala de Comando da Usina Nuclear</h1>", unsafe_allow_html=True)
col1, col2, col3 = st.columns(3)
with col1:
    st.write(' ')
with col2:
    st.image("https://media.tenor.com/BzAR7zFS0WUAAAAM/homer-simpson.gif")
with col3:
    st.write(' ')

st.markdown("<h2 style='text-align: center; color: red;'>Tempo Restante</h2>", unsafe_allow_html=True)
...
tentativas_restantes = 70 - st.session_state.tentativas
...
while True:
    ...
    if tempo_passado.seconds % 4 == 2 and not st.session_state['senha_correta']:
        adicionar_mensagem_erro()
        # Exibe as mensagens de erro
        placeholder_erros.empty()  # Limpa o placeholder antes de adicionar as mensagens atualizadas
    ...
    if st.button("Enviar senha para o controle"):
        if senha_digitada == senha_correta:
            ...
            st.success("Adequando cinética de fissão!........................100%")
            time.sleep(2)
            st.success("Reestabelecendo sistema de resfriamento!........................100%")
            time.sleep(4)
            st.success("Dispositivos de controle de sistemas online!........................100%")
            time.sleep(3)
            st.success("Temperatura normal reestabelecida!")
            time.sleep(2)
            st.success("Processo de normalização concluído com sucesso!")
            time.sleep(2)  # Intervalo de 2 segundos entre as mensagens
            st.image("https://i.imgur.com/bWMjh3w.jpg", use_column_width=True)
            placeholder_erros.empty()  # Limpa as mensagens de erro
            break  # Sai do loop se a senha estiver correta
        else:
            st.session_state['tentativas'] += 1
            if st.session_state['tentativas'] >= 70:
